...
public class MediaResource {
  ...
  public Response uploadImage(@FormDataParam("file") InputStream fileInputStream,
    ...
    Picture picture = new Picture(user, createLikes(), relativePath.toString());
    pc.create(picture);
    ...
    try {
      ...
      // Generate big image
      Thumbnails.of(file).size(1200, 800).outputFormat("jpg")
          .toFile(new File(file.getParent() + "/" + "big"));

      // Generate thumbnail
      Thumbnails.of(file).crop(Positions.CENTER).size(200, 200).outputFormat("jpg")
  ...
  public Response uploadProfileImage(@FormDataParam("file") InputStream fileInputStream,
    ...
    LOG.warning("Got user: " + user.getUsername());
    // Add new picture to the database
    Picture picture = new Picture(null, createLikes(), relativePath.toString());
    pc.create(picture);
  ...
  public JsonArrayBuilder createPictureArray(List<Picture> pictures,
      List<List<Picture>> albumPictures, List<Album> albums) {
    JsonArrayBuilder builder = Json.createArrayBuilder();
    for (Picture p : pictures) {
      builder.add(Json.createObjectBuilder()
          .add("path", p.getImagePath() + "/" + p.getId() + "/thumbnail.jpg").add("id", p.getId())
          .add("type", "image").add("time", p.getUploaded().getTimeInMillis()));
    }
    ...
    for (List<Picture> pList : albumPictures) {
      ...
      for (int i = 0; i < pList.size() && i < 4; i++) {
        innerBuilder.add(Json.createObjectBuilder()
            .add("path",
                pList.get(i).getImagePath() + "/" + pList.get(i).getId() + "/thumbnail.jpg")
